"""
Comprehensive Plotting for GPT Window Model Comparison
Optimized for 10+ runs with clear visualization
Creates 10 publication-quality plots from experimental results

Usage:
    from create_plots_improved import create_all_plots
    create_all_plots(all_baseline_metrics, all_window_metrics)
"""
...
def create_all_plots(baseline_metrics, window_metrics, output_dir='plots'):
    ...
    print("\n" + "=" * 80)
    print(f"CREATING 10 PUBLICATION-QUALITY PLOTS ({n_runs} runs)")
    print("=" * 80 + "\n")

    # Plot 1: Training loss curves with mean and confidence interval
    print("1. Training loss curves with mean...")
    plot_training_curves(baseline_metrics, window_metrics, f'{output_dir}/1_training_curves.png')

    # Plot 2: Final losses comparison with mean
    print("2. Final 100 losses comparison...")
    plot_final_losses(baseline_metrics, window_metrics, f'{output_dir}/2_final_losses.png')

    # Plot 3: Metrics comparison
    print("3. Metrics bar charts...")
    plot_metrics_comparison(baseline_metrics, window_metrics, f'{output_dir}/3_metrics_comparison.png')

    # Plot 4: Speed vs quality tradeoff
    print("4. Speed-quality tradeoff...")
    plot_speedup_quality(baseline_metrics, window_metrics, f'{output_dir}/4_speedup_quality.png')

    # Plot 5: Memory vs quality tradeoff
    print("5. Memory-quality tradeoff...")
    plot_memory_quality(baseline_metrics, window_metrics, f'{output_dir}/5_memory_quality.png')

    # Plot 6: Memory vs speed tradeoff
    print("6. Memory-speed tradeoff...")
    plot_memory_speed(baseline_metrics, window_metrics, f'{output_dir}/6_memory_speed.png')

    # Plot 7: Statistical distributions
    print("7. Statistical distributions...")
    plot_distributions(baseline_metrics, window_metrics, f'{output_dir}/7_distributions.png')

    # Plot 8: Per-run breakdown
    print("8. Per-run breakdown...")
    plot_per_run(baseline_metrics, window_metrics, f'{output_dir}/8_per_run.png')

    # Plot 9: Summary plot
    print("9. Improvement summary...")
    plot_summary(baseline_metrics, window_metrics, f'{output_dir}/9_summary.png')

    # Plot 10: Smoothed convergence with confidence bands
    print("10. Loss convergence with confidence bands...")
    plot_convergence(baseline_metrics, window_metrics, f'{output_dir}/10_convergence.png')
...
def plot_final_losses(baseline_metrics, window_metrics, save_path):
    ...
    baseline_mean = np.mean([np.pad(x, (0, max_len_b - len(x)), 'edge') 
                            for x in baseline_last100_all], axis=0)
    window_mean = np.mean([np.pad(x, (0, max_len_w - len(x)), 'edge') 
                          for x in window_last100_all], axis=0)
...
def plot_speedup_quality(baseline_metrics, window_metrics, save_path):
    ...
    # Bottom left: Slower & Better Quality
...
def plot_distributions(baseline_metrics, window_metrics, save_path):
    ...
    # Box plot
    ax = axes[0]
    bp = ax.boxplot([baseline_times, window_times], labels=['Baseline', 'Window'],
...
def plot_summary(baseline_metrics, window_metrics, save_path):
    ...
    # Adjust subplot to reduce margins
    ...
    for bar, val in zip(bars, improvements):
        width = bar.get_width()
        label_x = width + (0.3 if width > 0 else -0.3)  # Closer offset
    ...
    # Set tighter x-axis limits to reduce whitespace
    max_val = max(abs(x) for x in improvements)
    ax.set_xlim(-max_val * 0.3, max_val * 1.2)

    # Summary box with clearer title
    if n_runs >= 10:
    ...
    # Create summary text with title
    textstr = (
...
if __name__ == "__main__":
    print("Import this module and call create_all_plots() with your metrics data")
    print("\nExample usage:")
    print("  from create_plots_improved import create_all_plots")
    print("  create_all_plots(all_baseline_metrics, all_window_metrics)")
