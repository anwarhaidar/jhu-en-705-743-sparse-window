...
def compare_models_runs(num_runs=10, randomize_order=True):
    """
    Improved comparison with multiple runs and order randomization.
    """
    for d in ['output', 'plots']:
    ...
    for run in range(num_runs):
        ...
        for model_type in training_order:
            print(f"\nTraining {model_type.upper()} Model (Run {run + 1})")
        ...
        print(f"\nRun {run + 1} completed in {run_time:.2f}s ({run_time/60:.2f} min)")
    ...
    with open(os.path.join('output', 'improved_comparison_results.txt'), 'w') as f:
        f.write(f"\nComparison Results ({num_runs} runs):\n")
...
if __name__ == "__main__":
    ...
    baseline_metrics, window_metrics = compare_models_runs(num_runs=10, randomize_order=True)
    # Create all plots
